class Photo
  ...
  def extract_geolocation
    ...
    return true unless img_lat && img_lng && lat_ref && lng_ref
    ...
    set_city(self.location)
    return true
  end

  def set_city(loc)
    result = Geocoder.search("#{loc.latitude},#{loc.longitude}").first
    city_name = result.city
    c = City.find_by(:name => city_name)
    unless c.nil?
      self.city = c
    else
      self.city = City.new(:name => city_name)
      self.city.location = loc
      self.city.save
    end
  end
